...
text-overflow:ellipsis;/* 当对象内文本溢出时显示省略标记(...) ；需与overflow:hidden;一 起使用。*/
    ...
    <div class="main-wrap">
        <div class="crumb-wrap">
            ...
            </i><a href="<?php echo U('Admin/index/index');?>">首页</a><span class="crumb-step">&gt;</span><span class="crumb-name">所有州</span></div>
        </div>
            ...
            <div class="search-content">
                <form action="/jscss/admin/design/index" method="post">
                    <table class="result-tab" width="100%">
                        ...
                        <?php if(is_array($state)): $i = 0; $__LIST__ = $state;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            <td class="tc"><input name="id[]" value="<?php echo ($vo["state_id"]); ?>" type="checkbox"></td>
                            <td>
                                <input name="ids[]" value="<?php echo ($vo["state_id"]); ?>" type="hidden">
                                <input class="common-input sort-input" name="ord[]" value="<?php echo ($vo["state_id"]); ?>" type="text">
                            </td>
                            <td><?php echo ($vo["state_id"]); ?></td>
                            <td><?php echo ($vo["name"]); ?></td>
                            <td><?php echo ($vo["state_introduce"]); ?></td>
                            <td><?php echo ($vo["state_pic"]); ?></td>
                            <td><?php echo ($vo["weather"]); ?></td>
                            <td>
                                <a class="link-update" href="<?php echo U('Admin/state/edit');?>/state_id/<?php echo ($vo["state_id"]); ?>">修改</a>
                                <a class="link-del" href="<?php echo U('Admin/state/destroy');?>/state_id/<?php echo ($vo["state_id"]); ?>">删除</a>
                            </td>
